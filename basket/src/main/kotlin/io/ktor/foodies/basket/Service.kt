...
interface BasketService {
    /** Gets the basket for the specified buyer. Returns an empty basket if none exists. */
    suspend fun getBasket(buyerId: String): CustomerBasket

    /**
     * Adds an item to the buyer's basket. If the menu item already exists, increments the quantity.
     *
    ...
    suspend fun updateItemQuantity(
        buyerId: String,
        itemId: String,
        request: ValidatedUpdateQuantity,
    ): CustomerBasket?
...
class BasketServiceImpl(
    ...
    override suspend fun addItem(buyerId: String, request: ValidatedAddItem): CustomerBasket? {
        ...
        val updatedBasket =
            if (existingItemIndex >= 0) {
                ...
                val updatedItem =
                    existingItem.copy(quantity = existingItem.quantity + request.quantity)
                val updatedItems =
                    basket.items.toMutableList().apply { set(existingItemIndex, updatedItem) }
    ...
    override suspend fun updateItemQuantity(
        ...
        val updatedItems = basket.items.toMutableList().apply { set(itemIndex, updatedItem) }
