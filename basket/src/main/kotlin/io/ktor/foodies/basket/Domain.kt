...
data class CustomerBasket(val buyerId: String, val items: List<BasketItem> = emptyList()) {
    fun totalPrice(): SerializableBigDecimal =
        items.sumOf { it.unitPrice * it.quantity.toBigDecimal() }
}

/**
 * Individual items within a basket. Uses denormalized data from the Menu service for display
 * purposes.
 */
@Serializable
...
@Serializable data class UpdateItemQuantityRequest(val quantity: Int)

// Validated domain objects

data class ValidatedAddItem(val menuItemId: Long, val quantity: Int)

data class ValidatedUpdateQuantity(val quantity: Int)
