...
internal fun RabbitContainer.config(queueName: String) =
    RabbitConfig(host, amqpPort, adminUsername, adminPassword, queueName)
...
@TestRegistering
context(ctx: RabbitContext)
internal fun TestSuite.testListener(
    name: String,
    block: suspend (String, ProfileWebhookEventListener) -> Unit,
) =
    test(name) {
        val queueName = name.split(" ").joinToString(separator = ".").lowercase()
        val connection = ctx.factory().newConnection()
        val listener =
            ProfileWebhookEventListener(ctx.container().config(queueName), lazy { connection })
        try {
