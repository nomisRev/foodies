...
/**
 * We are making some assumptions here about 'Event', and it would be better to do a proper
 * integration test with a real Keycloak instance. This is non-trivial and is postponed for a proper
 * integration test of the entire system. Contributions welcome!
 */
val toUserEventSpec by
    ctxSuite(context = { rabbitContext() }, testConfig = TestConfig.testScope(true, 3.minutes)) {
        testListener("publishes NewUserEvent to RabbitMQ when REGISTER event is received") {
            queueName,
            listener ->
            val event =
                Event().apply {
        ...
        testListener(
            "publishes UpdateProfileEvent to RabbitMQ when UPDATE_PROFILE event is received"
        ) { queueName, listener ->
            val event =
                Event().apply {
        ...
        testListener(
            "publishes DeleteUserEvent to RabbitMQ when DELETE_ACCOUNT event is received"
        ) { queueName, listener ->
            val event =
                Event().apply {
            ...
            factory().channel { channel ->
                ...
                assertNotNull(response, "Expected a message in the queue")
                assertEquals(
                    UserEvent.Delete(subject = "delete-user-123"),
                    Json.decodeFromString<UserEvent>(response.body.decodeToString()),
                )
            }
        }

        testListener("publishes NewUserEvent with null optional fields when details are missing") {
            queueName,
            listener ->
            val event =
                Event().apply {
        ...
        testListener("does not publish message when event type is not REGISTER") {
            queueName,
            listener ->
            val event =
                Event().apply {
        ...
        testListener("throws when DELETE_ACCOUNT event has null userId") { queueName, listener ->
            ...
            val ex = assertFailsWith<IllegalStateException> { listener.onEvent(event) }
            assertEquals("Missing required fields for delete event: userId=null", ex.message)
        }
        ...
        testListener("close is idempotent") { _, listener -> listener.close() }
    }
