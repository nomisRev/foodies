...
private suspend fun fetchTokens(username: String, password: String): TokenResponse {
    val tokenEndpoint =
        "${ctx.keycloakContainer().authServerUrl}/realms/foodies-keycloak/protocol/openid-connect/token"
...
val userSessionScopeSpec by
    ctxSuite(context = { serviceContext() }, testConfig = TestConfig.testScope(true)) {
        testWebAppService("returns 401 and HX-Redirect when no session") {
            application { routing { userSession { get("/protected") { call.respondText("OK") } } } }
