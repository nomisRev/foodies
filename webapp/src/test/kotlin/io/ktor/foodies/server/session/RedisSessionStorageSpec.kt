...
val redisSessionStorageSpec by
    ctxSuite(context = { redisContext() }) {
        ...
        testRedis("read non-existent session throws exception") { storage ->
            assertFailsWith<NoSuchElementException> { storage.read("non-existent") }
        }
        ...
        testRedis("invalidate removes session") { storage ->
            ...
            assertFailsWith<NoSuchElementException> { storage.read("session-2") }
        }
        ...
        testSuite(name = "real-time", testConfig = TestConfig.testScope(false)) {
            testRedis("session expires after TTL", ttlSeconds = 1) { storage ->
                ...
                assertFailsWith<NoSuchElementException> { storage.read("session-3") }
            }
            ...
            testRedis("reading session does not refresh TTL", ttlSeconds = 2) { storage ->
                ...
                assertFailsWith<NoSuchElementException> { storage.read("session-4") }
            }
        }
    }
