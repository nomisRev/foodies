...
val serviceTokenSpec by testSuite {
    test("ServiceToken is not expired when time is before expiry") {
        val token =
            ServiceToken(accessToken = "token-123", expiresAt = Clock.System.now() + 5.minutes)
    ...
    test("ServiceToken is expired when time is past expiry") {
        val token =
            ServiceToken(accessToken = "token-456", expiresAt = Clock.System.now() - 1.seconds)
    ...
    test("ServiceToken is expired when within buffer window") {
        val token =
            ServiceToken(accessToken = "token-789", expiresAt = Clock.System.now() + 29.seconds)
    ...
    test("ServiceToken is not expired when beyond buffer window") {
        val token =
            ServiceToken(accessToken = "token-abc", expiresAt = Clock.System.now() + 31.seconds)
    ...
    test("ServiceToken uses default 30 second buffer") {
        val token =
            ServiceToken(accessToken = "token-def", expiresAt = Clock.System.now() + 25.seconds)
    ...
    test("ServiceToken has Bearer as default token type") {
        val token =
            ServiceToken(accessToken = "token-ghi", expiresAt = Clock.System.now() + 5.minutes)
    ...
    test("ServiceToken equality is based on all fields") {
        ...
        val token1 =
            ServiceToken(accessToken = "token-mno", expiresAt = expiresAt, tokenType = "Bearer")
        val token2 =
            ServiceToken(accessToken = "token-mno", expiresAt = expiresAt, tokenType = "Bearer")
