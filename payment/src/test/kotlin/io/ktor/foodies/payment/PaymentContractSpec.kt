 ...
 *
 * These tests verify the complete flow from processing payments through the service layer to actual
 * PostgreSQL storage and event publishing, ensuring all components work together correctly.
 */
val paymentContractSpec by
    ctxSuite(context = { serviceContext() }) {
        ...
        testPaymentService("idempotency: processing same order twice returns existing payment") {
            module ->
            val request =
                ProcessPaymentRequest(
                    eventId = "evt-002",
