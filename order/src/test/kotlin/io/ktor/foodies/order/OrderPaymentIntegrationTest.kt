...
val orderPaymentIntegrationSpec by testSuite {
    test("Full flow: Order Submission -> Stock Confirmation -> Payment -> Paid") {
        ...
        stockConfirmedHandler.handle(
            StockConfirmedEvent(order.id, Instant.fromEpochMilliseconds(0))
        )
...
private fun createTestOrder(status: OrderStatus) =
    Order(
        id = 1,
        requestId = "req-1",
        buyerId = "user-1",
