...
val gracefulCancellationSpec by testSuite {
    test("cancelling order in StockConfirmed state should publish stock return information") {
        ...
        val order =
            ctx.orderRepository.create(
                CreateOrder(
                    ...
                    items = listOf(CreateOrderItem(1L, "Burger", "url", BigDecimal("10.00"), 2)),
                    totalPrice = BigDecimal("20.00"),
                    currency = "USD",
                    paymentDetails =
                        PaymentDetails(CardBrand.VISA, "1234567812345678", "John", "123", 12, 2030),
                )
            )
        ...
        assertTrue(
            ctx.eventPublisher.stockReturnedEvents.isNotEmpty(),
            "Expected StockReturnedEvent to be published",
        )
