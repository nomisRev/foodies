...
import io.ktor.foodies.order.placement.RabbitPlacementEventPublisher
...
val fulfillmentEventPublisherSpec by testSuite {
    ...
    test("publish OrderCreatedEvent - successfully publishes message to RabbitMQ") {
        ...
        rabbit().newConnection().use { connection ->
            connection.createChannel().use { channel ->
                ...
                val placementEventPublisher = RabbitPlacementEventPublisher(publisher)
                placementEventPublisher.publish(event)
            }
        }
    ...
    test("publish OrderCancelledEvent - successfully publishes message to RabbitMQ") {
        ...
        rabbit().newConnection().use { connection ->
            connection.createChannel().use { channel ->
                val publisher = Publisher(channel, exchangeName, Json)
                val fulfillmentEventPublisher = RabbitFulfillmentEventPublisher(publisher)
                fulfillmentEventPublisher.publish(event)
            }
        }
    ...
    test("publish GracePeriodExpiredEvent - successfully publishes message to RabbitMQ") {
        ...
        rabbit().newConnection().use { connection ->
            connection.createChannel().use { channel ->
                val publisher = Publisher(channel, exchangeName, Json)
                val placementEventPublisher = RabbitPlacementEventPublisher(publisher)
                placementEventPublisher.publish(event, 5.seconds)
            }
        }
