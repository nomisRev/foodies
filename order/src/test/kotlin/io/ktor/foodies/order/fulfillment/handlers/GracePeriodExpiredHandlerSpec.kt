...
val gracePeriodExpiredHandlerSpec by testSuite {
    test("GracePeriodExpiredHandler should transition order to AwaitingValidation") {
        val ctx = createTestContext()
        val order = createHandlerTestOrder(OrderStatus.Submitted)
        ctx.orderRepository.orders.add(order)
        val handler = GracePeriodExpiredHandler(ctx.fulfillmentService)
