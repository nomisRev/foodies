...
fun Application.module(config: Config, telemetry: OpenTelemetry): OrderModule {
    ...
    val eventPublisher =
        RabbitOrderEventPublisher(Publisher(rabbitChannel, config.rabbit.exchange, Json))
    val orderRepository = ExposedOrderRepository(dataSource.database)
    val orderService =
        DefaultOrderService(orderRepository, basketClient, eventPublisher, config.order)
