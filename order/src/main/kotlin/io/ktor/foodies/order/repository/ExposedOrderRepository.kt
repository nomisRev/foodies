...
class ExposedOrderRepository(private val database: Database) : OrderRepository {
    override fun findById(id: Long): Order? =
        transaction(database) {
            val orderRow =
                Orders.selectAll().where { Orders.id eq id }.singleOrNull()
                    ?: return@transaction null
            val items =
                OrderItems.selectAll().where { OrderItems.orderId eq id }.map { it.toOrderItem() }
            val history =
            ...
            val paymentMethod =
                paymentMethodId?.let { pmId ->
                    PaymentMethods.selectAll()
                        .where { PaymentMethods.id eq pmId }
    ...
    override fun findByRequestId(requestId: String): Order? =
        transaction(database) {
            val orderRow =
                Orders.selectAll().where { Orders.requestId eq requestId }.singleOrNull()
                    ?: return@transaction null
    ...
    override fun findAll(
        ...
        transaction(database) {
            ...
            val orders =
                query.orderBy(Orders.createdAt, SortOrder.DESC).limit(limit).offset(offset).map {
                    row ->
    ...
    override fun create(order: CreateOrder): Order =
        transaction(database) {
            ...
            val paymentMethodId =
                PaymentMethods.selectAll()
                    .where {
                        (PaymentMethods.buyerId eq order.buyerId) and
                            (PaymentMethods.cardType eq order.paymentDetails.cardType) and
            ...
            val returning =
                Orders.insertReturning(listOf(Orders.id, Orders.createdAt, Orders.updatedAt)) {
    ...
    private fun ResultRow.toOrderHistoryEntry() =
        OrderHistoryEntry(
