    ...
    ctxSuite(context = { serviceContext() }) {
        ...
        listOf(null, "name")
            ...
            .forEach { updateRequest ->
                testMenuService(
                    "Update existing menu item with ${Json.encodeToString(updateRequest)}"
                ) {
                    val created =
                        jsonClient()
                            .post("/menu") {
                    ...
                    val response =
                        jsonClient().put("/menu/${created.id}") {
                            contentType(ContentType.Application.Json)
                    ...
                    assertEquals(
                        updateRequest.description ?: created.description,
                        updated.description,
                    )
