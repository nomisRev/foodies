...
fun Route.menuRoutes(menuService: MenuService) {
    ...
    // New route for service-to-service communication
    authenticate("service-to-service-auth-jwt") {
        route("/menu/internal") {
            get("/status") {
                call.respondText("Menu service internal status: OK", status = HttpStatusCode.OK)
            }
        }
    }
}